...
def train(args):
    ...
    for i in range(args.num_encs):
        ...
        #print('stream{}: input dims : {}'.format(i + 1, idim_list[i]))
    ...
    for key in sorted(vars(args).keys()):
        ...
        #print('ARGS: ' + key + ': ' + str(vars(args)[key]))
    ...
    if args.ngpu > 1:
        if args.batch_size != 0:
            ...
            #print('batch size is automatically increased (%d -> %d)' % (
            #    args.batch_size, args.batch_size * args.ngpu))
...
#    device = torch.device("cuda" if args.ngpu > 0 else "cpu")
    device = "cuda:"+str(args.local_rank) if args.dist_train else torch.device("cuda" if args.ngpu > 0 else "cpu")
    ...
    #model.cuda()
    if args.dist_train:
        model = torch.nn.parallel.DistributedDataParallel(model,[args.local_rank], output_device=args.local_rank)
    ...
    train_iter = {'main': ChainerDataLoader(
        ...
        shuffle=(train_sampler is None), collate_fn=lambda x: x[0], sampler=train_sampler)}
    ...
    #trainer.extend(extensions.LogReport(trigger=(args.report_interval_epochs, 'epoch')))
    ...
    if args.opt == 'adadelta':
        ...
        trainer.extend(extensions.observe_value(
            'eps', lambda trainer: trainer.updater.get_optimizer('main').param_groups[0]["eps"]),
            trigger=(args.report_interval_epochs, 'epoch'))
    ...
    trainer.extend(extensions.PrintReport(
        report_keys), trigger=(args.report_interval_epochs, 'epoch'))

    trainer.extend(extensions.ProgressBar(update_interval=args.report_interval_iters))
    #trainer.extend(extensions.ProgressBar(update_interval=args.report_interval_epochs))
    ...
    if args.batchsize == 0:
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                token_as_list = [train_args.char_list[idx] for idx in tokenid_as_list]
                print(token_as_list)
